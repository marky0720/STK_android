...
namespace os
{
	...
	{
#ifdef _IRR_COMPILE_WITH_ANDROID_DEVICE_
        __android_log_print(ANDROID_LOG_INFO, "irrPrinter::print", "%s", message);
...
namespace os
{
	...
	void Printer::log(const wchar_t* message, ELOG_LEVEL ll)
	{
        ...
        __android_log_print(ANDROID_LOG_INFO, "log","%s", msg.c_str());
#else
		if (Logger)
			Logger->log(message, ll);
#endif
...
#ifdef _IRR_COMPILE_WITH_ANDROID_DEVICE_
        __android_log_print(ANDROID_LOG_INFO, "log", "%s %s",message,hint);
#else
		if (Logger)
...
#ifdef _IRR_COMPILE_WITH_ANDROID_DEVICE_
        __android_log_print(ANDROID_LOG_INFO, "log","%s %s", message,hint.c_str());
#else
		if (Logger)
