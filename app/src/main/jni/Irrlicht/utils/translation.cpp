...
wchar_t*  MByteToWChar (char* lpcstr)
{
	wchar_t* pszOut = NULL; 
	if (lpcstr != NULL)
	{
		...
		pszOut = new wchar_t [nOutputStrLen]; 
		if(pszOut)
		{
...
wchar_t* GB2312_To_Unicode(const char* gb2312,int & len)
{  
	//gb2312תunicode
	wchar_t* w=MByteToWChar((char*)gb2312);
...
const wchar_t* Translations::w_gettext(const char* original)
{
	...
	wchar_t ar[512]= L"";
		mbstowcs(ar,original,strlen(original));

 /*   if (original[0] == '\0') return L"";
    int out=1;
	return GB2312_To_Unicode(original,out);*/
