...
{
	ALOGV("CFileSystem::getAbsolutePath begin");
	DBG_LINE;
	...
	DBG_LINE;
	ALOGV("filename [%s] %d",filename.c_str() ,filename.size() );
	if (filename.size()>0 &&  filename[filename.size()-1]=='/')
	{
		ALOGV("CFileSystem::getAbsolutePath end22");
		return io::path(p)+"/";
	}
	else
	{
		ALOGV("CFileSystem::getAbsolutePath end33");
		return io::path(p);
	}
#else
	...
	ALOGV("CFileSystem::getAbsolutePath end");
}
