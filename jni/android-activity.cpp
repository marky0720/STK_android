...
void input_key(bool pressed,EKEY_CODE k)
{
	   // 模拟键盘输出事件
...
//	device->getCursorControl()->setPosition(x,y);
    ...
    if (action==0) {
	    ...
	    //模拟
	    input_key(true,KEY_UP);

	    if(x<100){
	    	 input_key(true,KEY_LEFT);
	    }
	    if(x>=100){
	    	input_key(true,KEY_RIGHT);
	    }
        ...
        //模拟
    	input_key(false,KEY_UP);


	    if(x<100){
	    	 input_key(false,KEY_LEFT);
	    }


	    if(x>=100){
	    	 input_key(false,KEY_RIGHT);
	    }
...
{
	ALOGV("nativeDrawIteration begin");
    nativeDrawIteration();   
	ALOGV("nativeDrawIteration end");
}
