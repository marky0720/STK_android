...
IrrDriver::IrrDriver()
{
	ALOGV("IrrDriver::IrrDriver begin");
	DBG_LINE;
...
{
	ALOGV("IrrDriver::initDevice begin");
	DBG_LINE;
...
*/
    firstTime = false;
	ALOGV("IrrDriver::initDevice 11111111111");
    /*
    int numDrivers = 5;
    ...
    */
    DBG_LINE;
    ALOGV("IrrDriver::initDevice 22222222222222");
    m_device = createDevice( video::EDT_OGLES1, dimension2d<u32>(
    		600,//UserConfigParams::m_width,
    		400 //UserConfigParams::m_height
    		), 16, false, false, false, 0);
    ALOGV("IrrDriver::initDevice 3333333333333333333333");
   	__android_log_print(ANDROID_LOG_INFO, "Irrlicht", "createDevice r=%d ", m_device   );
    ...
    if(!m_device)
    {
    	 ALOGV("IrrDriver::initDevice 3333333333333333333");
        fprintf(stderr, "Couldn't initialise irrlicht device. Quitting.\n");
        exit(-1);
    }
    ALOGV("IrrDriver::initDevice 444444444444444444");
    DBG_LINE;
  ...
  //  m_device->setWindowCaption(L"SuperTuxKart");
    ...
    ALOGV("IrrDriver::initDevice 5555555555555555555");
    ...
    ALOGV("IrrDriver::initDevice 666666666666666666");
    DBG_LINE;
    ...
    ALOGV("IrrDriver::initDevice 77777777777777777777777");
    DBG_LINE;
	...
	ALOGV("IrrDriver::initDevice end");
}
...
void IrrDriver::update(float dt)
{
    ...
    ALOGV("IrrDriver::update 22222222222222");
    ...
    ALOGV("IrrDriver::update 333333333333333");
    if (world != NULL)
    {
    ...
    ALOGV("IrrDriver::update 4444444444444444");
    {   // Just to mark the begin/end scene block
        ...
        ALOGV("IrrDriver::update 55555555555555555");
        if (inRace)
        {
            ...
            for(unsigned int i=0; i<world->getNumKarts(); i++)
            {
                ...
                if(kart->getCamera()) 
                {
                	ALOGV("IrrDriver::update 66666666666666666");
                    kart->getCamera()->activate();
                    ALOGV("IrrDriver::update activate 666666666");
                    m_scene_manager->drawAll();
                    ALOGV("IrrDriver::update 77777777777777777");
            ...
            ALOGV("IrrDriver::update 8888888888888888888");
            // To draw the race gui we set the viewport back to the full
            // screen. 
            //
//         ±»×¢ÊÍ   m_video_driver->setViewPort(core::recti(0, 0,
//                UserConfigParams::m_width,
//                UserConfigParams::m_height));
            for(unsigned int i=0; i<world->getNumKarts(); i++)
            {
                Kart *kart = world->getKart(i);
               //±»×¢ÊÍ if(kart->getCamera())
                //    rg->renderPlayerView(kart);
            }  // for i<getNumKarts

            ALOGV("IrrDriver::update 9999999999999999");
        }
        else
        {
        ...
        ALOGV("IrrDriver::update xxxxxxxxxxxxxxxxxxxxx");
        ...
        //FIXME   if(!inRace || !UserConfigParams::m_bullet_debug)
        {
            // Either render the gui, or the global elements of the race gui.
        	 ALOGV("IrrDriver::update  GUIEngine::render( begin");
            GUIEngine::render(dt);
            ALOGV("IrrDriver::update  GUIEngine::render( end");
        }
